<template>
  <div class="sidepanel-app">
    <header v-if="moduleMeta" class="sidepanel-header">
    ...
    <main class="sidepanel-content">
      <div v-if="loading" class="loading-state">
        <el-icon class="is-loading" size="32px"><Loading /></el-icon>
        <p>加载中...</p>
      </div>
      <div v-else-if="error" class="error-state">
        <el-icon size="32px"><Warning /></el-icon>
        <p>{{ error }}</p>
      </div>
...
onMounted(async () => {
  try {
    // 从 storage 读取模块名称
    const result = await chrome.storage.session.get(['currentSidepanelModule'])
    moduleName.value = typeof result.currentSidepanelModule === 'string' ? result.currentSidepanelModule : ''

    if (!moduleName.value) {
      error.value = '未找到要加载的模块'
      loading.value = false
      return
    }
    ...
    if (!raw?.route?.component) {
      error.value = `模块 "${moduleName.value}" 未找到或未配置组件`
      loading.value = false
      return
    }
